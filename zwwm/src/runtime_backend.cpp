...
renderer::Rect assigned_tile(const Surface& surface) {
  ...
  const auto right = static_cast<std::int32_t>(std::lround(
      surface.camera_center_x + (static_cast<double>(base.origin.x) + base.size.width - surface.camera_center_x) * scale));
  const auto bottom = static_cast<std::int32_t>(std::lround(
      surface.camera_center_y + (static_cast<double>(base.origin.y) + base.size.height - surface.camera_center_y) * scale));
...
renderer::Rect animated_content(const Surface& surface, const renderer::Rect& tile) {
  ...
  const auto right = project_x(static_cast<std::int64_t>(surface.assigned_content_x) + surface.assigned_content_width);
  const auto bottom = project_y(static_cast<std::int64_t>(surface.assigned_content_y) + surface.assigned_content_height);
...
WindowGeometry window_geometry(const Surface& surface) {
  ...
  const int right = static_cast<int>(std::clamp<std::int64_t>(static_cast<std::int64_t>(surface.geometry_x) + surface.geometry_width, left, surface.width));
  const int bottom = static_cast<int>(std::clamp<std::int64_t>(static_cast<std::int64_t>(surface.geometry_y) + surface.geometry_height, top, surface.height));
